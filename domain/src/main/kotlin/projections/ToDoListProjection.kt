...
class ToDoListProjection(eventFetcher: FetchStoredEvents<ToDoListEvent>) :
    ...
    companion object {
        fun eventProjector(e: ToDoListEvent): List<DeltaRow<ToDoListProjectionRow>> =
            when (e) {
                is ListCreated -> CreateRow(e.rowId(), ToDoListProjectionRow(e.owner, true, ToDoList(e.name, emptyList())))
        ...
        private fun <T : Any> DeltaRow<T>.toSingle(): List<DeltaRow<T>> = listOf(this)
    }
}
